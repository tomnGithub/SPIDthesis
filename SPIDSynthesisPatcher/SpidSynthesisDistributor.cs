...
internal sealed class SpidSynthesisDistributor
{
    ...
    public SpidSynthesisDistributor(
    ...
    public void Run()
    {
        ...
        Console.WriteLine($"SPID Synthesis: found {iniFiles.Length} *_DISTR.ini file(s) in {_state.DataFolderPath}.");
        if (iniFiles.Length == 0) return;

        var parsedRules = SpidIniParser.ParseFiles(iniFiles, Warn);
        Console.WriteLine($"SPID Synthesis: parsed {parsedRules.Count} Keyword/Spell/Perk rule(s).");
        ...
        Console.WriteLine($"SPID Synthesis: resolved {resolvedRules.Length} rule(s); {_rulesSkipped} rule(s) skipped.");

        if (_settings.VerboseLogging || resolvedRules.Length <= 25)
        {
            Console.WriteLine("SPID Synthesis: resolved form filters:");
        ...
        Console.WriteLine($"SPID Synthesis: scanned {npcRecordsScanned} winning NPC record(s) in template-aware mode.");
        ...
        {
            Console.WriteLine(
                $"SPID Synthesis: {rulesWithoutMatches} rule(s) matched no NPCs. " +
                "Enable verbose logging for per-rule counts.");
        }

        Console.WriteLine("SPID Synthesis complete.");
    ...
    private bool AddKeyword(FormKey keyword, NpcEvaluationState npc)
    {
        ...
        if (npc.InheritsKeywords && !npc.KeywordsMaterialized)
        {
    ...
    {
        if (!npc.InheritsSpellList || npc.SpellListMaterialized) return true;
